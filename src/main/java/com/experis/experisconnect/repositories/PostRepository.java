...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(value = "select * from Post p where p.parent_id is null and p.users_id = ?1 and ((lower(p.title) like (%?2%) or (lower(p.content) like (%?2%)))) ORDER BY created_at DESC limit ?3 offset ?4 ", nativeQuery = true)
